...
disp('Here using explicit SBP operators of orders 2,4 and 6')


disp(' '); 
disp('The red line is the magnetic component and the blue the electric component. An animation of the numerical solution is generated and stored as a mp4-file.')
...
% 
  ...
  ordning=0;
  while (ordning ~=2 && ordning ~=4 && ordning ~=6)
  ordning = input('Order of accuracy of SBP operator (2)(4)(6): ');
  disp(' '); 
  end
      ...
      BC=0; % Type of outher BC
  while (BC ~=1 && BC ~=2 && BC ~=3)
  BC = input('Type of BCs: (1) Electric component, (2) Magnetic component (3) Absorbing BCs : ');
  disp(' '); 
  end
...
epsilon_l=1;
mu_l=1;
cl=1/sqrt(epsilon_l*mu_l);

%epsilon_r=1;
mu_r=1;
cr=1/sqrt(epsilon_r*mu_r);
n_l=sqrt(epsilon_l);
n_r=sqrt(epsilon_r);
...
vidObj = VideoWriter('Maxwell2.mp4','MPEG-4');
open(vidObj);
...
yb_d=-2;yb_u= 1.3;
    ...
    elseif BC == 2
        L=[kron(e2,e_1') BC_0;
           ...
           BC_I];
    elseif BC == 3
        % L=[kron(e1+cl*e2,e_1') BC_0;
        %    BC_0 kron(e2-cr*e1,e_m'); 
        %    BC_I];
        L=[kron(e1-cl*e2,e_1') BC_0;
           BC_0 kron(e1+cr*e2,e_m'); 
           BC_I];
    end
